...
class Minesweeper:
    ...
    """def level(self, won):
        if won==1:
            level_window= Tk()
            level_window.title("난이도 선택")
            level_window.geometry("400X400")
            label = Label(level_window, text = )
            label.pack()
            score = """""
    ...
    def __init__(self, tk):
        ...
        self.labels["남은 지뢰"].grid(row = SIZE_X+1, column = int(SIZE_Y/3)-1, columnspan = int(SIZE_Y/2)) # middle
    ...
    def gameOver(self, won):
        ...
        if won==1:
            ...
            #여기서 문자열 잘 해서 바꾸면 될 것 같은디 안되네  
            for i in range(len(rank_list)):
                score = " "
                score = score + "나는 " + str(ranking+1)+ "등!! \n" + '\n'.join(rank_list)
